...
Sub CrearTextoNumSerieEnA3()
    ...
    Dim FontNumSerie As String
    
    ValorNegro = Val(frmMain.txt_NivelNegroCodificacion.Value)
    FontNumSerie = frmMain.txt_FontNumSerie.Value
    ...
    For Each s In sr
        ...
        Set shDATA_OBJECT1 = ActiveDocument.MasterPage.Layers("Layer3").CreateArtisticText(s.PositionX + 74.341, s.PositionY - 2.563, NumSerie, , , FontNumSerie, 14, cdrFalse, , , cdrCenterAlignment)
    Next s
...
Sub SampleINIRespaldoObligatorias()
    ...
    sReturn = sManageSectionEntry(iniWrite, "Obligatorias", "FontNumSerie", sINI_FILE, frmMain.txt_FontNumSerie.Value)
